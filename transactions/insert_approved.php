...
try {
    ...
    $trans_amount = htmlspecialchars(trim($data->trans_amount));
    $bal = 0;
    $new_bal = 0;
    $user_row;
    ...
    if($stmt->execute()){
        ...
        echo json_encode([
            'success' => 0,
            'message' => 'New transaction NOT Inserted.'
        ]);
        exit;
    ...
    // Insert and update transaction to transaction table
    ...
    if ($stmt2->execute()) {
        ...
        // Update User balance on bankusers table
        $update_user_bal_query = "UPDATE `bankusers` SET balance = :balance WHERE id = :id";
        $stmt3 = $conn->prepare($update_user_bal_query);

        $stmt3->bindValue(':balance', $new_bal, PDO::PARAM_INT);
        $stmt3->bindValue(':id', $user_id, PDO::PARAM_INT);
        ...
        if($stmt3->execute()){
            ...
            echo json_encode([
                'success' => 0,
                'message' => 'User balance update failed. Update manually.',
            ]);
            exit;
        ...
        http_response_code(400);
            echo json_encode([
                'success' => 0,
                'message' => 'Transaction update failed. Update Transaction and user balance manually.',
            ]);
            exit;
    }
